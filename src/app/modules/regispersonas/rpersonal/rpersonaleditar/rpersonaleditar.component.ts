...
@Component({
  selector: 'app-rpersonaleditar',
  templateUrl: './rpersonaleditar.component.html',
  styleUrls: ['./rpersonaleditar.component.scss']
})
export class RpersonaleditarComponent {
  //array
  ...
  ngOnInit() {
    ...
    this.http.getrpersona(this.id_person).subscribe(data => {
  ...
  editarRpersona(nombre:string, apellido: string, edad: string, academico : string,telefono: string, direccion: string){
    let editarRpersona: any;
      ...
      this.http.editarPersona(editarRpersona).subscribe(result=>{
        if(result.message!==''){
