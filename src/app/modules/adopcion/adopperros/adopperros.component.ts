...
@Component({
  selector: 'app-adopperros',
  templateUrl: './adopperros.component.html',
  styleUrls: ['./adopperros.component.scss']
})
export class AdopperrosComponent {
  ...
  generarpdf(id_animal: number) {
    this.http.getRperro(id_animal).subscribe(data => {
      if (data !== null) {
  ...
  generatePDF() {
    ...
    pdf.open();
    this.router.navigate([`adopperros`]);
  }
