...
@Component({
  selector: 'app-rperroseditar',
  templateUrl: './rperroseditar.component.html',
  styleUrls: ['./rperroseditar.component.scss']
})
export class RperroseditarComponent {
    ...
    ngOnInit() {
      ...
      this.http.getRperro(this.id_anim).subscribe(data => {
    ...
    editarRperroo(nombre1:string, edad1: string, raza1 : string){
      let editarRpersona: any;
        editarRpersona = {
          ...
          this.http.editarRperro(editarRpersona).subscribe(result=>{
            if(result.message!==''){
