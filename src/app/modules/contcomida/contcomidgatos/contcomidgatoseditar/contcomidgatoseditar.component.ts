...
@Component({
  selector: 'app-contcomidgatoseditar',
  templateUrl: './contcomidgatoseditar.component.html',
  styleUrls: ['./contcomidgatoseditar.component.scss']
})
export class ContcomidgatoseditarComponent {
     ...
     ngOnInit() {
       ...
       this.http.getCCgato(this.idtb_registro_comida).subscribe(data => {
     ...
     editarRpersona(cantidad_ingreso:number, fecha_ingreso: string){
       let editarRpersona: any;
         ...
         this.http.editarCCgato(editarRpersona).subscribe(result=>{
           if(result.message!==''){
